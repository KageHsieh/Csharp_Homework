...
namespace Form_Loan
{
    public partial class FormStudent_Struct : Form
    {
        ...
        private void buttonHighlow_Click(object sender, EventArgs e)
        {
            if (textBoxChi.Text == "" || textBoxEng.Text == "" || textBoxMath.Text == "")
            {
                MessageBox.Show("請輸入正確數值做比對!","提醒",MessageBoxButtons.OK,MessageBoxIcon.Information);
                return;
            }
            ...
            if (Chi < Eng && math == Eng)
            {
                groupBoxHighLow.Text = $"最高科目成績為 : 英文和數學{math}分\n最低科目成績為 : 國文{Chi}分";
            }

            if (Eng < Chi && math == Chi)
            {
                groupBoxHighLow.Text = $"最高科目成績為 : 國文和數學{math}分\n最低科目成績為 : 英文{Eng}分";
            }

            if (math < Chi && Eng == Chi)
            {
                groupBoxHighLow.Text = $"最高科目成績為 : 國文和英文{Chi}分\n最低科目成績為 : 數學{math}分";
            }
